...
# Remove the large dark margin before sampling so the logo fills its panel.
mask = gray.point(lambda value: 255 if value > 32 else 0)
bounds = mask.getbbox()
if bounds:
    gray = gray.crop(bounds)
gray.thumbnail((COLS, ROWS), Image.Resampling.LANCZOS)
canvas = Image.new("L", (COLS, ROWS), 0)
canvas.paste(gray, ((COLS - gray.width) // 2, (ROWS - gray.height) // 2))
gray = canvas
...
for index, line in enumerate(lines):
    delay = index * 0.035
    rows.append(
        f'<text class="row" x="205" y="{72 + index * 9.2:.1f}" '
...
<style>.row{{fill:#f0883e;font:9.2px ui-monospace,Consolas,monospace;white-space:pre;opacity:0;animation:type .28s ease forwards}}@keyframes type{{from{{opacity:0;transform:translateX(-7px)}}to{{opacity:1;transform:none}}}}</style>
